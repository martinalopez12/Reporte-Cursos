...
if check_password():
    ...
    def load_data(sheet_name):
        ...
        try:
            ...
            st.error(f"No se pudo cargar la hoja {sheet_name}. Verificá los permisos.")
            return pd.DataFrame()
    ...
    for df in [df_pagos, df_historico]:
        ...
        if "Contacto Viejo" in df.columns:
            df["Contacto Viejo"] = df["Contacto Viejoaten"].fillna("").astype(str)
    ...
    if opcion == "1. Pendientes del Mes":
        ...
        cursos_del_mes = df_pendientes["Curso"].unique()
        ...
        if len(cursos_del_mes) == 0:
            ...
            for curso in list(cursos_del_mes):
                ...
                # REGLA DEL PUNTITO VERDE: Si tiene filas donde 'Notif. Andre' es True, le ponemos el puntito
